...
const pullScratchBlocks = async (apiKey) => {
	...
	await rimraf(path.join(__dirname, '..', '..', 'data', 'i18n', 'scratch-blocks'))
	await fs.mkdir(path.join(__dirname, '..', '..', 'data', 'i18n', 'scratch-blocks'), { recursive : true })
	...
	await Promise.all(locales.map(async (code) => {
		...
		return fs.writeFile(
			path.join(__dirname, '..', '..', 'data', 'i18n', 'scratch-blocks', `${code}.json`),
