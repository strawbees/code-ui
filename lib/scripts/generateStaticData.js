	...
	templates.reduce(async (resolution, template) => {
		...
		if (localeIndex === 0) {
			templateData = PO.parse(
				await fs.readFile(
					path.join(__dirname, '..', '..', 'data', 'i18n', 'templates', `${template}.pot`),
					'utf8'
				)
			...
			templateData = PO.parse(
				await fs.readFile(
					path.join(__dirname, '..', '..', 'data', 'i18n', 'translations', template, `${locale}.po`),
					'utf8'
				)
...
const buildMdData = (templates, locale, localeIndex) =>
	templates.reduce(async (resolution, template) => {
		...
		let templateData = (await fs.readFile(
			path.join(__dirname, '..', '..', 'data', 'i18n', 'md.templates', `${template}.md`),
			'utf8'
		)).toString()
		...
		if (localeIndex > 0) {
			try {
				templateData = (await fs.readFile(
					path.join(__dirname, '..', '..', 'data', 'i18n', 'md.translations', template, `${locale}.md`),
					'utf8'
				)).toString()
			} catch (error) {
			}
...
const buildScratchBlocksData = async (locale) =>
	JSON.parse(await fs.readFile(
		path.join(__dirname, '..', '..', 'data', 'i18n', 'scratch-blocks', `${locale}.json`),
		'utf8'
	))
const generateI18nFiles = async (i18nData) => {
	// build a fresh i18n directory
	await rimraf(path.join(__dirname, '..', '..', 'static', 'i18n'))
	await fs.mkdir(path.join(__dirname, '..', '..', 'static', 'i18n'), { recursive : true })

	// save the i18n index
	await fs.writeFile(
		path.join(__dirname, '..', '..', 'static', 'i18n', 'index.json'),
	...
	await LOCALES.reduce(async (resolution, locale) => {
		await resolution
		return fs.writeFile(
			path.join(__dirname, '..', '..', 'static', 'i18n', `${locale}.json`),
...
const generateRoutesFiles = async (i18nData) => {
	...
	const routesTemplate = JSON.parse(
		await fs.readFile(path.join(__dirname, '..', '..', 'data', 'routes.json'))
	)
	...
	await rimraf(path.join(__dirname, '..', '..', 'static', 'routes.js'))
...
module.exports = async () => {
	...
	const i18nData = await buildI18nData(
		LOCALES,
		(await fs.readdir(path.join(__dirname, '..', '..', 'data', 'i18n', 'templates')))
			.filter(filename => filename.endsWith('.pot'))
			.map(filename => filename.replace('.pot', '')),
		(await fs.readdir(path.join(__dirname, '..', '..', 'data', 'i18n', 'md.templates')))
