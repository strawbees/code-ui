...
execute(async ({ exec }) => {
	try {
		...
		// await exec(`webpack -c ${path.resolve(__dirname, '..', '..', 'webpack.lib-quirkbot.config.js')}`)
		...
		const rootDest = path.resolve(NEXT_EXPORT_PATH, path.join.apply(null, ROOT_PATH.split('/')))
