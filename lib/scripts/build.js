...
execute(async ({ exec }) => {
	try {
		...
		await exec(`next export -o ${NEXT_EXPORT_PATH}`)
		// stop the static server
		staticServer.close()
		// move the resources to the root path destination
		const rootDest = path.join(NEXT_EXPORT_PATH, path.join.apply(null, ROOT_PATH.split('/')))
		await fs.rename(path.join(NEXT_EXPORT_PATH, '_next'), path.join(rootDest, '_next'))
		await fs.rename(path.join(NEXT_EXPORT_PATH, 'static'), path.join(rootDest, 'static'))
