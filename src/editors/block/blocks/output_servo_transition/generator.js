...
export default ({ value, next }, structure) => {
	...
	structure.body += ';\n'
	structure.body += `float c = ${getBlockBody(positionBlock, structure)} - b;\n`
	structure.body += `float d = ${getBlockBody(durationBlock, structure)};\n`
	...
	structure.body += '}\n'
	parseInstacePropertyAssignmentFromValue(structure, instance, 'position', getBlockBody(positionBlock, structure))
	structure.body += '}\n'
